...
function ability.caleValueNumber(obj, whichLevel, key, percent, fixed)
    ...
    if (type(obj.bindUnit) == "function") then
        ---@type Unit
...
function ability.casting(whichAbility, triggerUnit, evtData)
    ...
    event.trigger(whichAbility, EVENT.Ability.Casting, evtData) --触发技能持续每周期做动作时
...
function ability.castStart(whichAbility, triggerUnit, evtData)
    ...
    event.trigger(whichAbility, EVENT.Ability.Effective, evtData) --触发技能被施放
    ...
    if (ck > 0) then
        ...
        triggerUnit:prop("abilityKeepTimer", time.setInterval(period, function()
            ...
            elseif (ti >= triggerUnit:prop("abilityKeepTimerEnd")) then
                ...
                if (type(revert) == "function") then
                    revert(false)
                    event.trigger(whichAbility, EVENT.Ability.Over, evtData) --触发持续技能结束
                    event.trigger(triggerUnit, EVENT.Ability.Over, evtData) --触发单位持续技能结束
                end
            else
                if (ti % 10 == 0) then
                    ability.casting(whichAbility, triggerUnit, evtData)
        ...
        triggerUnit:onEvent(EVENT.Unit.InterruptIn, "cast_", function(interruptInData)
            event.unregister(triggerUnit, EVENT.Unit.InterruptIn, "cast_")
...
function ability.castStop(whichAbility, triggerUnit, evtData)
    event.trigger(whichAbility, EVENT.Ability.Stop, evtData) --触发技能被停止
    event.trigger(triggerUnit, EVENT.Ability.Stop, evtData) --触发单位停止施放技能
end
...
function ability.effective(whichAbility, evtData)
    ...
    --- 技能开始施放（但未生效）
    event.trigger(whichAbility, EVENT.Ability.Spell, evtData)
