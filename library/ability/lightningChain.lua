...
local _lc = function(index, lightningType, sourceUnit, prevUnit, targetUnit, dmg, dmgSrc, dmgType, dmgTypeLv, breakArmor)
    local sp
    ...
    Lightning(lightningType, 0.25, sp[1], sp[2], sp[3], targetUnit:x(), targetUnit:y(), targetUnit:h())
    sp = nil
    ability.damage({
...
function ability.lightningChain(options)
    ...
    if (damage > 0) then
        ...
        local index = 1
        _lc(index, lightningType, sourceUnit, nil, targetUnit, damage * rate * 0.01, dmgSrc, dmgType, dmgTypeLv, breakArmor)
        qty = qty - 1
        if (qty > 0) then
            ...
            time.setInterval(0.25, function(curTimer)
                ...
                _lc(index, lightningType, sourceUnit, targetUnit, nextUnit, damage * rate * 0.01, dmgSrc, dmgType, dmgTypeLv, breakArmor)
                qty = qty - 1
                targetUnit = nextUnit
            end)
        end
