...
function class:content(data)
    ...
    if ((type(icons) == "table" and #icons > 0)) then
        -- 当有顶部图标时，自动调节为左对齐
        japi.FrameSetTextAlignment(childText:handle(), TEXT_ALIGN_LEFT)
    elseif (type(tips) == "table" and #tips == 1) then
        -- 当只有一行文本时，自动调节为居中
        japi.FrameSetTextAlignment(childText:handle(), TEXT_ALIGN_CENTER)
    else
        -- 无图标且多行文本时，以代码设定为准
        local ag = childText:textAlign()
        japi.FrameSetTextAlignment(childText:handle(), ag)
    end
    ...
    for i = 1, #childIcons, 1 do
        if (type(icons) ~= "table" or icons[i] == nil) then
            ...
            cw = cw + 0.013 + mbstring.viewWidth(icons[i].text, fs)
        end
    end
    tw = math.max(tw, cw)
    if (hasIcons) then
    ...
    for i = 1, #childBars, 1 do
        if (type(bars) ~= "table" or bars[i] == nil) then
            ...
            lastBarIdx = i
            local bh = mbstring.viewHeight(1, fs - 2)
            if (i == 1) then
    ...
    if (type(tips) == "table" and #tips > 0) then
        ...
        for _, s in ipairs(tips) do
            s = i18n.tr(s)
            txw = math.max(txw, mbstring.viewWidth(s .. ' ', fs))
            txts[#txts + 1] = s
            ns = ns + string.subCount(s, "|n")
        end
        local txh = mbstring.viewHeight(#txts + ns, fs)
