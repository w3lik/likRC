...
function class:findPosition(x, y)
    local nx = 0
    local ny = 0
    local d = 32
    if (x ~= nil and y ~= nil) then
        ...
        nx = x
        ny = y
        local pd = self:prop("positionData")
        local ik = x .. '_' .. y
        local bic = 0
        local bor = 1
        local bi = 0
...
---@private
---@return void
function class:freePosition(x, y)
    if (x ~= nil and y ~= nil) then
        local pd = self:prop("positionData")
        x = math.round(x)
        y = math.round(y)
    ...
    if (type(param) == "table") then
        ...
        for idx, key in ipairs(param) do
            keyboard.onRelease(KEYBOARD[key], "abilityHotkey", function(evtData)
                ...
                Cursor():abilityQuote(abilitySlot:storage()[idx])
            end)
        end
    ...
    if (type(param) == "table") then
        ...
        for idx, key in ipairs(param) do
            keyboard.onRelease(KEYBOARD[key], "itemHotkey", function(evtData)
                ...
                Cursor():itemQuote(itemSlot:storage()[idx])
            end)
        end
