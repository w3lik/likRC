...
function class:construct(options)
    ...
    self:prop("mapLv", math.max(1, japi.Map_GetMapLevel(self:handle()) or 1))
    self:prop("isRedVip", japi.Map_IsRedVIP(self:handle()) or false) -- 红VIP
    self:prop("isBlueVip", japi.Map_IsBlueVIP(self:handle()) or false) -- 蓝VIP
    self:prop("isPlatformVIP", japi.Map_IsPlatformVIP(self:handle()) or false) -- 平台VIP
    self:prop("apm", 0)
...
function class:onSelectUnit(n, callFunc)
    event.register(self, EVENT.Player.SelectUnit .. "#" .. n, callFunc)
    return self
end
...
function class:onSelectItem(n, callFunc)
    event.register(self, EVENT.Player.SelectItem .. "#" .. n, callFunc)
    return self
end
...
function class:onChat(pattern, callFunc)
    if (false == event.has(self, EVENT.Player.Chat)) then
        event.register(self, EVENT.Player.Chat, function(evtData)
            ---@type Array
...
function class:hasMallItem(mallItemKey)
    ...
    if (DEBUGGING) then
        return self:prop("allMallItem") or false
    end
    return japi.Map_HasMallItem(self:handle(), mallItemKey) or false
end
...
--- 所有选中目标
---@return table{number,Unit|Item}
function class:selections()
    local g = J.CreateGroup()
    J.handleRef(g)
    J.GroupEnumUnitsSelected(g, self:handle(), nil)
    local ts = {}
    while (true) do
        local u = J.FirstOfGroup(g)
        if (u <= 0) then
            break
        end
        J.GroupRemoveUnit(g, u)
        table.insert(ts, h2o(u))
    end
    J.GroupClear(g)
    J.DestroyGroup(g)
    J.handleUnRef(g)
...
---@return self|string
function class:alert(modify, vcm)
    if (type(modify) == "string" and string.len(modify) > 0) then
        if (type(vcm) ~= "boolean") then
            vcm = true
        end
        if (vcm) then
            audio(Vcm("war3_Error"), self)
        end
        local alertTimer = self:prop("alertTimer")
        if (isClass(alertTimer, TimerClass)) then
            self:clear("alertTimer", true)
        end
        self:prop("alert", modify)
        local t = math.max(3, 0.2 * mbstring.len(modify))
        self:prop("alertTimer", time.setTimeout(t, function()
            self:clear("alertTimer")
            self:prop("alert", "")
        end))
        return self
    end
    return self:prop("alert")
end