...
function class:construct(options)
    ...
    self:propChange("weightCur", "std", 0, false)
...
function class:speechAlias(modify)
    if (modify) then
        if (isDestroy(self) == false and self:isAlive()) then
            return self:prop("speechAlias", modify)
        end
    end
...
--- 是否硬直中
---@return boolean
function class:isPunishing()
    return self:superposition("punish") > 0
end
...
function class:orderRouteDestroy()
    event.register(self, EVENT.Unit.MoveRoute, "RalOrderRoute", nil)
    ...
    if (isClass(targetItem, ItemClass) and targetItem:instance() == true) then
        player._unitDistanceAction(self, targetItem, Game():itemActionDistance(), function()
            if (targetItem:instance() == true) then
                ...
                event.trigger(targetItem, EVENT.Item.Pick, { triggerItem = targetItem, triggerUnit = self })
                if (isClass(targetItem, ItemClass)) then
                    event.trigger(self, EVENT.Item.Pick, { triggerItem = targetItem, triggerUnit = self })
                    if (isClass(targetItem, ItemClass)) then
