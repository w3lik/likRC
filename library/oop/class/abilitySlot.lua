...
function class:triggerChange()
    ...
    if (isClass(bu, UnitClass)) then
        if (bu:owner():selection() == bu) then
            event.trigger(bu, EVENT.Unit.AbilitySlotChange, { triggerSlot = self, triggerUnit = bu })
        end
    end
end
...
function class:insert(whichAbility, index)
    ...
    if (isClass(prev, AbilityClass)) then
        ...
        if (swap > 0) then
            ...
            s[index] = ab:bindUnit(self:bindUnit()):abilitySlotIndex(index)
            event.trigger(ab, EVENT.Ability.Get, { triggerUnit = self:bindUnit(), triggerAbility = ab })
            if (isClass(ab, AbilityClass)) then
                event.trigger(self:bindUnit(), EVENT.Ability.Get, { triggerUnit = self:bindUnit(), triggerAbility = ab })
            end
        end
    else
        if (ab:bindUnit() == self:bindUnit()) then
            ...
            s[index] = ab:bindUnit(self:bindUnit()):abilitySlotIndex(index)
            event.trigger(s[index], EVENT.Ability.Get, { triggerUnit = self:bindUnit(), triggerAbility = s[index] })
            if (isClass(s[index], AbilityClass)) then
                event.trigger(self:bindUnit(), EVENT.Ability.Get, { triggerUnit = self:bindUnit(), triggerAbility = s[index] })
            end
        end
    end
    self:triggerChange()
...
function class:remove(index)
    ...
    if (type(index) == "number" and index > 0 and index <= self:volume()) then
        ---@type Ability
        if (isClass(s[index], AbilityClass)) then
            event.trigger(s[index], EVENT.Ability.Lose, { triggerUnit = self:bindUnit(), triggerAbility = s[index] })
            if (isClass(s[index], AbilityClass)) then
                event.trigger(self:bindUnit(), EVENT.Ability.Lose, { triggerUnit = self:bindUnit(), triggerAbility = s[index] })
            end
