...
function class:construct(options)
    if (options.frameId == nil) then
        ...
        options.frameId = japi.CreateFrameByTagName(options.fdfType, options.tag, options.parent:handle(), options.fdfName, 0)
...
function class:destruct()
    if (isClass(view._evtAdaptive, ArrayClass)) then
        view._evtAdaptive:set(self:id(), nil)
    end
    -- 清理事件
    local evtList = event.data(self)
    ...
    japi.DestroyFrame(self._frameId)
end
...
function class:adaptive(modify)
    if (type(modify) == "boolean") then
        local _, updated = self:PROP("adaptive", modify)
        if (updated == true) then
            local rel = self:relation()
            if (type(rel) == "table") then
                self:relation(table.unpack(rel))
            end
            local s = self:size()
            if (type(s) == "table") then
                self:size(table.unpack(s))
            end
            local child = self:children()
            if (type(child) == "table") then
...
function class:isFocus()
    return japi.GetMouseFocus() == self:handle()
end
        ...
        if (delay >= 0.1) then
            ...
            if (isClass(self, FrameTooltipsClass)) then
                self:onEvent(EVENT.Frame.Hide, "tactics_", function(evtData)
                    event.unregister(evtData.triggerFrame, EVENT.Frame.Hide, "tactics_")
                    event.unregister(evtData.triggerFrame, EVENT.Frame.Enter, "tactics_")
                    event.unregister(evtData.triggerFrame, EVENT.Frame.Leave, "tactics_")
                end)
            ...
            self:prop("showDelay", time.setTimeout(delay, function()
...
function class:gradientStop()
    ...
    if (isClass(t, TimerClass)) then
        ...
        if (type(oriRelation) == "table") then
            japi.FrameSetPoint(self:handle(), oriRelation[1], oriRelation[2]:handle(), oriRelation[3], oriRelation[4], oriRelation[5])
        end
        local oriSize = self:size()
        if (type(oriSize) == "table") then
            japi.FrameSetSize(self:handle(), oriSize[1], oriSize[2])
        end
        japi.FrameSetAlpha(self:handle(), self:alpha())
        japi.FrameShow(self:handle(), self:show())
    end
end
...
function class:gradient(options, callback)
    ...
    if (type(options.alpha) == "number") then
        ...
        if (curAlpha ~= nil) then
            japi.FrameSetAlpha(self:handle(), curAlpha)
        end
    end
    ...
    if (dtX ~= 0 or dtY ~= 0) then
        japi.FrameClearAllPoints(self:handle())
        japi.FrameSetPoint(self:handle(), oriRelation[1], oriRelation[2]:handle(), oriRelation[3], oriRelation[4] + dtX * step, oriRelation[5] + dtY * step)
    end
    if (dtW ~= 0 and dtH ~= 0) then
        japi.FrameSetSize(self:handle(), oriSize[1] + dtW * step, oriSize[2] + dtH * step)
    end
    japi.FrameShow(self:handle(), true)
    local t = time.setInterval(options.duration / step, function(curTimer)
        step = step - 1
        if (step <= 0) then
