...
function class:texture(modify)
    if (modify) then
        modify = AUIKit(self:kit(), modify, "tga")
    end
...
function class:duration(modify)
    if (type(modify) == "number" and modify > 0) then
        ...
        if (isClass(stepTimer, TimerClass)) then
...
function class:isPlaying()
    return isClass(self:prop("stepTimer"), TimerClass)
end
...
function class:play(loop, isReset)
    ...
    local frequency = self:duration() / #m
    stepTimer = time.setInterval(frequency, function(curTimer)
        ...
        if (motion[step] == nil) then
            ...
            if (halt > 0) then
                destroy(curTimer)
                self:prop("step", 0)
                self:prop("stepTimer", time.setTimeout(halt, function()
