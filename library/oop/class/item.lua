...
function class:onUnitEvent(evt, ...)
    ...
    if (key ~= nil) then
        ...
        if (callFunc == nil) then
            event.unregister(self, EVENT.Item.Get, eKey)
            event.unregister(self, EVENT.Item.Lose, eKey)
        else
...
function class:isAlive()
    return self:superposition("dead") <= 0
end
    ...
    player._unitDistanceAction(u, { x, y }, Game():itemActionDistance(), function()
        ...
        time.setTimeout(0.15, function()
            if (false == isDestroy(self)) then
                self:position(x, y)
                event.trigger(u, EVENT.Item.Drop, { triggerItem = self, triggerUnit = u, targetX = x, targetY = y })
                event.trigger(self, EVENT.Item.Drop, { triggerItem = self, triggerUnit = u, targetX = x, targetY = y })
            end
        end)
    end)
end
        ...
        if (slot) then
            ...
            player._unitDistanceAction(u, { x, y }, Game():itemActionDistance(), function()
                ...
                time.setTimeout(0.15, function()
                    if (false == isDestroy(self)) then
                        slot:insert(self)
                        event.trigger(u, EVENT.Item.Deliver, { triggerItem = self, triggerUnit = u, targetUnit = targetUnit })
                        event.trigger(self, EVENT.Item.Deliver, { triggerItem = self, triggerUnit = u, targetUnit = targetUnit })
                    end
                end)
            end)
        end
...
function class:pawn()
    ...
    if (isClass(bp, PlayerClass)) then
        ...
        bp:worth("+", rp)
        event.trigger(self, EVENT.Item.Pawn, { triggerItem = self, triggerUnit = self:bindUnit() })
        event.trigger(self:bindUnit(), EVENT.Item.Pawn, { triggerItem = self, triggerUnit = self:bindUnit() })
    end
    destroy(self)
end
