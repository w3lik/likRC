...
expander["interrupt"] = function(obj)
    obj:superpositionCale("interrupt",
        function()
            event.trigger(obj, EVENT.Unit.InterruptIn, { triggerUnit = obj })
        end, function()
            event.trigger(obj, EVENT.Unit.InterruptOut, { triggerUnit = obj })
        end)
end
...
expander["noAttack"] = function(obj)
    obj:superpositionCale("noAttack",
        function()
            japi.UnitDisableAttack(obj:handle(), true)
        end, function()
            japi.UnitDisableAttack(obj:handle(), false)
        end)
end
