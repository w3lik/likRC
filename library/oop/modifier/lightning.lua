...
expander["lightningType"] = function(obj)
    ...
    local point = obj:propData("point")
    local data = obj:propData("lightningType")
    obj._handle = J.AddLightningEx(data.value, false, table.unpack(point))
end

---@param obj Lightning
expander["point"] = function(obj)
    ---@type number[]
    local data = obj:propData("point")
