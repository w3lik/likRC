    ...
    if (key) then
        ...
        if (Game():playingQuantityStart() > 1) then
            ...
            if (sync._timer[aid] == nil) then
                sync.query(aid)
                sync._timer[aid] = time.setInterval(0.1, function(curTimer)
...
function sync.receive(key, callback)
    if (sync._isInit ~= true) then
        ...
        J.handleRef(tgr)
        japi.TriggerRegisterSyncData(tgr, "ralsync", false)
        J.TriggerAddAction(tgr, function()
            local pid = 1 + J.GetPlayerId(japi.GetTriggerSyncPlayer())
            for _, v in ipairs(string.explode("||", japi.GetTriggerSyncData())) do
