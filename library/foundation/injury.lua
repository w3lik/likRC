    ...
    if (sourceUnit:isMelee()) then
        ...
        if (isClass(am, AttackModeClass)) then
            ...
            if (mode == "lightning") then
                ...
                for _ = 1, focus do
                    Lightning(am:lightningType(), lDur, x1, y1, z1, x2, y2, z2)
...
function injury.arriveItem(sourceUnit, targetItem)
    ...
    if (dmg >= 1) then
        ...
        event.trigger(targetItem, EVENT.Item.Be.Attack, {
            triggerItem = targetItem,
            sourceUnit = sourceUnit,
            damage = dmg,
        })
...
function injury.killItem(deadItem)
    ...
    --- 触发死亡事件
    event.trigger(deadItem, EVENT.Item.Dead, { triggerItem = deadItem, sourceUnit = deadItem:lastHurtSource() })
